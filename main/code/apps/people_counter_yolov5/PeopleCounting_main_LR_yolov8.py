...
while vid.isOpened():
    ...
    roi = Polygon([(p1.x-120, p1.y), (p2.x-120, p2.y), (p2.x+120, p2.y), (p1.x+120, p1.y)])
    ...
    # If the model ouputs normalized coordinates, scael them back
    if np.max(h_output) <= 1: # Assuming h_output is a numpy array
        h, w = frame.shape[:2] # Assuming frame is the original image
        h_output = h_output * [w, h, w, h, 1, 1] # Asumming h_output is in [x_min, y_min, x_max, y_max, confidence, class] format
    ...
    for i, det in enumerate(pred): # Detections per image
        print(det) # [x1, y1, x2, y2, object confidence, ID]
        if len(det) != 0:
            if status == "Waiting":
                status = "Tracking"
        else:
