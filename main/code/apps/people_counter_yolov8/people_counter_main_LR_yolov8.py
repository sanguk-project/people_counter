...
h_output = np.empty(settings['onnx_shape'], dtype=np.float32) # Assuming n classes + 5 bounding box attributes, input_shape(512, 512) → 16,128
...
objects_moving_left = 0
objects_moving_right = 0
...
while vid.isOpened():
    ...
    p1 = Point(frame.shape[1] // 2 - 550, 0)
    p2 = Point(frame.shape[1] // 2 + 550, frame.shape[0])

    # Create a Polygon from these points
    roi = Polygon([(p1.x-50, p1.y), (p2.x-50, p2.y), (p2.x+50, p2.y), (p1.x+50, p1.y)])

    # Prepare the frame for inference
    img = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    img_resized = cv2.resize(img, (settings['img_size'], settings['img_size']))
    img_transpose = np.transpose(img_resized, (2, 0, 1)).astype(np.float32)
    img_transpose /= 255.0
    img_expand = np.expand_dims(img_transpose, axis=0)
    img_asarray = np.ascontiguousarray(img_expand)

    # Copy input image to GPU memory
    cuda.memcpy_htod_async(d_input, img_asarray, stream)
    ...
    h_output_tensor = torch.from_numpy(h_output)

    # Apply NMS
    pred = non_max_suppression(h_output_tensor, conf_thres=settings['conf_thres_nms'], iou_thres=settings['iou_thres_nms'], classes=None, agnostic=None, nc=1, max_wh=1920)
    ...
    for i, det in enumerate(pred): # Detections per image
        ...
        for (objectID, centroid) in objects.items():
            ...
            # Otherwise, there is a trackable object so we can utilize it
                ...
                # Create a Point object from the centroid
                    ...
                    if direction_x < 0 and within:
                        totalLeft += 1
                        to.counted = True

                    elif direction_x > 0 and within:
                        totalRight += 1
                        to.counted = True
        ...
        total_object_counts = totalLeft + totalRight
        ...
        tracking_accuracy_left = totalLeft / objects_moving_left * 100 if objects_moving_left != 0 else 0
        tracking_accuracy_right = totalRight / objects_moving_right * 100 if objects_moving_right != 0 else 0
        ...
        for to in trackableObjects.values():
            if len(to.centroids) >= 2:
                ...
                if to.centroids[-1][0] < to.centroids[-2][0]: # Moving left
                    if to.disappeared <= settings['maxDisappeared']:
                        objects_moving_left += 1
                        to.disappeared = settings['maxDisappeared'] + 1 # Set disappeared to a value greater than maxDisappeared 
                elif to.centroids[-1][0] > to.centroids[-2][0]: # Moving Right
                    if to.disappeared <= settings['maxDisappeared']:
                        objects_moving_right += 1
                        to.disappeared = settings['maxDisappeared'] + 1 # Set disappeared to a value greater than maxDisappeared 
        ...
        info = [
            ...
            ("Objects Moving Left", objects_moving_left),
            ("Objects Moving Right", objects_moving_right),
            ("Counting Accuracy", "{:.2f}%".format(tracking_accuracy)),
            ("Counting Accuracy Left", "{:.2f}%".format(tracking_accuracy_left)),
            ("Counting Accuracy Right", "{:.2f}%".format(tracking_accuracy_right)),
            ("Left", totalLeft),
            ("Right", totalRight),   
        ]
        ...
        for (i, (k, v)) in enumerate(info):
            ...
            if i == 10: # Counting Accuracy_Right
            ...
            # For "Left", we place it in the bottom left corner
            if i == 11: # Left
                cv2.putText(frame, key_text, (10, frame.shape[0] - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(frame, value_text, (10 + key_text_width, frame.shape[0] - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

            # For "Right", we place it in the bottom left corner
            if i == 12: # Right
